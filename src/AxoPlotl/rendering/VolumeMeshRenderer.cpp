...
void VolumeMeshRenderer::create_buffers(const StaticData &_data)
{
    ...
    // Vertex Index Buffer
    {
        ...
        desc.size = sizeof(uint32_t) * _data.vertex_instances_.size();
        desc.mappedAtCreation = false;
    ...
    // Edge Index Buffer
    {
        ...
        desc.size = sizeof(EdgeInstance) * _data.edge_instances_.size();
        desc.mappedAtCreation = false;
