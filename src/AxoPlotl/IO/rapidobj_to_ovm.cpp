...
{
    OVMVolumeMesh ovm;
    auto v_shape_i = ovm.create_persistent_vertex_property<int>("rabidobj::v_shape", -1).value();
    auto e_shape_i = ovm.create_persistent_edge_property<int>("rabidobj::e_shape", -1).value();
    auto f_shape_i = ovm.create_persistent_face_property<int>("rabidobj::f_shape", -1).value();
    auto f_normal = ovm.create_persistent_face_property<OVMVolumeMesh::Point>("rabidobj::f_normal", {0,0,0}).value();
    using Vec3 = OVMVolumeMesh::Point;

    // Positions
    ovm.reserve_vertices(_robj.attributes.positions.size()/3);
    ...
    for (unsigned int shape_i = 0; shape_i < _robj.shapes.size(); ++shape_i) {
        ...
        for (const auto& n_inds : _robj.shapes.at(shape_i).lines.num_line_vertices) {
            ...
            for (int i = 0; i < vhs.size()-1; ++i) {
                OVM::EH eh = ovm.add_edge(vhs[i], vhs[i+1]);
